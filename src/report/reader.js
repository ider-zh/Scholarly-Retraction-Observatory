...
const NAMES = {
  ...
  'author-top': 'OpenAlex 作者身份关联排行 · Top 20', 'rw-author-names': 'RW 原始署名字符串关联排行 · Top 20', 'source-counts': '主要发表来源', 'publisher-counts': '当前出版商关联数量',
...
export function populationName(chart) {
  if (isBroadChart(chart)) {
    const names = {A1: '宽口径撤稿标记候选', D: '同口径全部发表文献', A1_over_D: '宽口径标记候选与全部发表文献比较', C_D_over_D: 'RW 记录候选与同口径发表文献比较'};
    if (names[chart.population_key]) return names[chart.population_key];
  }
  return POPULATIONS[chart.population_key] || '本图研究范围';
}

export function variantName(chart) {
  const parts = [populationName(chart)];
...
const countryDisplayOverrides = {TW: '中国台湾', HK: '中国香港', MO: '中国澳门'};
export function countryLabel(code) {
  return `${countryDisplayOverrides[code] || countryNames.of(code)}（${code}）`;
}
export function rowLabel(row, chart) {
  if (chart.chart_id === 'G1' && /^[A-Z]{2}$/.test(row.id)) return countryLabel(row.id);
...
export function chartMethod(chart) {
  if (isBroadChart(chart) && ['screening', 'work-types', 'Q3'].includes(chart.chart_id)) return [BROAD_WORK_METHOD, '分子与全部发表文献分母同步采用此规则；旧 article 严格口径不得与新宽口径分母混用。保留不确定候选会减少漏计，但不能保证所有候选都是真正的原论文。'];
  ...
  if (chart.chart_id === 'rw-author-names') return chart.limitations;
