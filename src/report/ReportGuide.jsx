...
const INTRO = {
  ...
  entities: ['先看署名关联，再理解排名', 'OpenAlex 的机构和作者按 ID 识别，RW 原始署名排行则按姓名字符串统计。同一姓名字符串可能对应多个 OpenAlex 作者 ID，不能直接当作同一个人的论文合计。关联数量只描述本研究范围内的署名，不是全部发文数，更不是不端行为排名。'],
...
export default function ReportGuide({page, charts}) {
  const [title, summary] = INTRO[page] || CHAPTERS[page].slice(1);
  const population = charts?.find(chart => chart.chart_id === 'population-accounting');
  const broad = charts?.some(isBroadChart);
  ...
  return <section className="snapshot-reader-guide" aria-label="阅读指南"><h2>{title}</h2><p>{summary}</p>
    {page === 'overview' && <>
      ...
      <details><summary>主体库、扩展库和 A0/A1 等代码是什么意思？</summary><p><strong>主体库（core）</strong>是 OpenAlex 区别于扩展库（expansion）的语料部分，由快照中的 is_xpac 字段区分。这里默认使用主体库以保持统计范围一致；“主体”不是质量认证，也不保证每条记录经过同行评审。</p><dl><dt>A0：带撤稿标记的记录</dt><dd>还未区分原论文和通知的数据库标记数量。</dd><dt>A1：筛选后的标记候选</dt><dd>{broad ? BROAD_WORK_METHOD : '从 A0 中保留符合文献类型、身份和日期规则的研究论文候选。'}</dd><dt>B：RW 撤稿原论文</dt><dd>按原论文标识去重后的 RW 记录；不同通知不自动算作多篇原论文。</dd><dt>C：匹配成功的记录</dt><dd>能与 OpenAlex 高置信对接的 RW 原论文对应记录；不要求 OpenAlex 已经标记撤稿。具体图表还会限定文献类型。</dd><dt>D：比较基数</dt><dd>同一范围内的全部合格发表论文，用来计算“每万篇中有多少篇”的比例。</dd></dl></details>
      {counts.A0 > 0 && <p className="snapshot-reader-finding">从主体库的 <strong>{number(counts.A0)} 条</strong>撤稿标记记录出发，默认分析保留 <strong>{number(counts.A1)} 条</strong>{broad ? '宽口径文献候选' : '研究论文候选'}（{number(100*counts.A1/counts.A0)}%）。其余记录没有从来源数据中删除；下方“筛选前后”会解释每一种排除及其比例。</p>}
    </>}
