...
export const DISCIPLINE_METRICS = {count: '撤稿记录论文数', share: '样本内论文覆盖比例（%）', rate: '每万篇发表论文中的记录数'};
...
export function resolveStudy(data, sources, selection) {
  ...
  if (!Object.hasOwn(DISCIPLINE_METRICS, metric) || (metric === 'rate' && taxonomy.denominator === null)) return {error: '该比率没有可用的同口径全体发文分母；不能用样本占比替代撤稿率。'};
  const root = {id: 'all', label: '全部主学科', level: -1, parents: [], counts: taxonomy.counts, denominator: taxonomy.denominator};
  ...
  const parent = selection.parent || node.parents[0] || '';
...
export function disciplineCell(taxonomy, node, population, metric, index) {
  ...
  const denominator = metric === 'rate' ? publicationDenominator : scope;
  ...
  return {id: node.id, label: node.label, numerator, denominator, value,
    ...
    ranking_eligible: metric !== 'rate' || (numerator >= 20 && publicationDenominator >= 1000)};
}

export function distributionRows(study) {
  const {taxonomy, node, parent, population, metric} = study;
  const candidates = taxonomy.nodes.filter(candidate => !candidate.navigation_only && (node.level === -1 ? candidate.level === 0 : candidate.parents.includes(node.level === 0 ? node.id : parent)));
  return candidates.map(candidate => disciplineCell(taxonomy, candidate, population, metric, 0)).sort((first, second) => {
    if (metric === 'rate' && (!first.ranking_eligible || !second.ranking_eligible)) return Number(second.ranking_eligible) - Number(first.ranking_eligible) || first.label.localeCompare(second.label);
...
export function disciplineTimeChart(data, study) {
  ...
  return {chart_id: 'discipline-time', slice_id: `${taxonomy.id}/${population}/${node.id}/${metric}`,
