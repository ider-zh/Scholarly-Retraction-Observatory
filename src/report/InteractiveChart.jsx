...
import {plotKind, plotRows, lineSegments, plotMaximum} from './chartGeometry.js';
import ChartRange from './ChartRange.jsx';

function axisLabel(chart, kind) {
  if (kind === 'scatter') return '全部发表论文数 N（对数刻度）';
  if (kind === 'control') return '撤稿记录月份';
  if (['discipline-time', 'country-time'].includes(chart.chart_id)) return '原论文发表年';
...
export default function InteractiveChart({chart}) {
  ...
  const maximumY = plotMaximum(chart, visible);
  ...
  const tickLabel = value => kind === 'scatter' ? number(Math.round(10 ** value)) : chart.chart_id === 'C1' ? number(Math.round(Math.expm1(value))) : kind === 'control' ? rows[Math.round(value)]?.label : ['T1', 'T3', 'discipline-time', 'country-time'].includes(chart.chart_id) ? String(value) : number(value);
  ...
  return <section className="snapshot-interactive-chart" aria-label={`${chartName(chart)}交互图表`}>
    ...
    {['line', 'control'].includes(kind) && rows.length > 2 && <ChartRange rows={rows} start={start} end={end} onChange={([nextStart, nextEnd]) => {setStart(nextStart); setEnd(nextEnd); setPinned(null); setHovered(null);}} onReset={reset}/>}
